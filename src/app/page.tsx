...
export default function Home() {
  const [hcns, setHcns] = useState<number[]>([]);
  const [copiedIndices, setCopiedIndices] = useState<Set<number>>(new Set());

  const handleGenerate = () => {
    const generated: number[] = [];
    for (let i = 0; i < 20; i++) {
      generated.push(genHCN());
    }
    setHcns(generated);
    setCopiedIndices(new Set()); // Clear copied when regenerating
  };

  const handleCopy = (hcn: number, index: number) => {
    navigator.clipboard.writeText(hcn.toString());
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {hcns.map((hcn, index) => (
              <div
                key={index}
                className={`text-xl font-mono p-4 rounded cursor-pointer transition-all duration-200 text-center
